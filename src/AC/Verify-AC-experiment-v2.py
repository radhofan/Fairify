...
# Define constraint for Adult dataset (assuming same as in first file)
constraint = np.array([
    [10, 100],    # age
    [0, 6],       # workclass
    [0, 15],      # education
...
# Protected attributes (sex is at index 8)
protected_attribs = [8]
num_attribs = len(constraint)

# Initialize separate debug counters for fairness evaluation
fairness_debug_counters = {
    'fallback_to_original': 0,
    f'sat_unfair_{FAIRER_MODEL_NAME.lower()}_used': 0,
    f'unsat_fair_{ORIGINAL_MODEL_NAME.lower()}_used': 0,
    f'unknown_{ORIGINAL_MODEL_NAME.lower()}_used': 0
...
# Create partition-based hybrid fairness evaluator
print(f"Creating PartitionBasedHybridFairnessEvaluator...")
partition_evaluator = PartitionBasedHybridFairnessEvaluator(
    original_model=original_model,
    fairer_model=fairer_model,
    constraint=constraint,
    partition_results=partition_results,
    debug_counters=fairness_debug_counters,
    original_name=ORIGINAL_MODEL_NAME,
    fairer_name=FAIRER_MODEL_NAME,
    protected_attribs=protected_attribs,
    num_attribs=num_attribs
)

# Test original model alone for comparison
print(f"\nTesting Original Model ({ORIGINAL_MODEL_NAME}) with discrimination generation:")
orig_avg, orig_interval, orig_stats = partition_evaluator.evaluate_individual_fairness_partition_based(
    sample_round=10, num_gen=100, use_hybrid=False
)

# Test partition-based hybrid switching
print(f"\nTesting Partition-Based Hybrid Model with discrimination generation:")
partition_avg, partition_interval, partition_stats = partition_evaluator.evaluate_individual_fairness_partition_based(
    sample_round=10, num_gen=100, use_hybrid=True
)

# Display comparison results
print(f"\n" + "="*70)
print("PARTITION-BASED HYBRID SWITCHING RESULTS COMPARISON")
print("="*70)
print(f"Original Model Discrimination Rate: {orig_avg:.4f} ± {orig_interval:.4f}")
print(f"Partition-Based Hybrid Discrimination Rate: {partition_avg:.4f} ± {partition_interval:.4f}")

if orig_avg > 0:
    improvement = orig_avg - partition_avg
    improvement_pct = (improvement / orig_avg) * 100
    print(f"Improvement: {improvement:.4f} ({improvement_pct:.1f}% reduction)")
else:
    print("Improvement: N/A (original rate is 0)")

if partition_stats["original"] + partition_stats["fairer"] > 0:
    total_partition_instances = partition_stats["original"] + partition_stats["fairer"]
    print(f"\nPartition-Based Hybrid Model Usage Statistics:")
    print(f"Total test instances generated: {total_partition_instances}")
    print(f"Used original model: {partition_stats['original']} ({partition_stats['original']/total_partition_instances*100:.1f}%)")
    print(f"Used fairer model: {partition_stats['fairer']} ({partition_stats['fairer']/total_partition_instances*100:.1f}%)")
    print(f"Discriminatory cases found: {partition_stats['discriminatory_cases']}")
    print(f"Discrimination rate with partition-based switching: {partition_stats['discriminatory_cases']/total_partition_instances*100:.2f}%")

# Print fairness evaluation debug counters
print(f"\nPartition-Based Fairness Evaluation Debug Counters:")
for key, value in fairness_debug_counters.items():
    print(f"{key}: {value}")
