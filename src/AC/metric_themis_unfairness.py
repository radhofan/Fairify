...
class CausalDiscriminationDetector:
    ...
    def fairness_improvement_lln(self, protected_features, test_cases, m=10000, T=100):
        """
        ...
        for trial in range(T):
            ...
            # Generate m samples for this trial
            for _ in range(m):
                # Use existing test cases if available, otherwise generate random
                if test_cases and len(test_cases) > 0:
                    base_case = random.choice(test_cases)
                else:
                    base_case = self._generate_random_assignment(self.input_order)
                
                # Get prediction for base case
                original_prediction = self._get_prediction(base_case)
                
                # Check if discriminatory by testing all protected attribute combinations
                fixed_features = [f for f in self.input_order if f not in protected_features]
                fixed_assignment = {f: base_case[f] for f in fixed_features}
                
                # Test all possible protected attribute assignments
                for alt_protected_assignment in self._generate_all_assignments(protected_features):
                    modified_case = {**fixed_assignment, **alt_protected_assignment}
                    
                    # Skip if it's the same as original
                    if all(modified_case[f] == base_case[f] for f in protected_features):
                        continue
...
if __name__ == "__main__":
    ...
    E_original = detector_orig.fairness_improvement_lln(['sex'], test_cases_orig, m=10000, T=100)
    E_repaired = detector_fair.fairness_improvement_lln(['sex'], test_cases_fair, m=10000, T=100)
