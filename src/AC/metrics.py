...
def generate_lhs_counterfactuals(model, feature_names, range_dict, num_samples=500, protected_attributes=['sex']):
    dim = len(feature_names)
    
    min_bounds = [range_dict[feat][0] for feat in feature_names]
    max_bounds = [range_dict[feat][1] for feat in feature_names]
    
    sampler = qmc.LatinHypercube(d=dim)
    lhs_raw = sampler.random(n=num_samples)
    lhs_scaled = qmc.scale(lhs_raw, min_bounds, max_bounds)
    
    lhs_scaled = np.round(lhs_scaled).astype(int)
    
    df = pd.DataFrame(lhs_scaled, columns=feature_names)
    
    predictions = model.predict(lhs_scaled)
    ...
    protected_indices = [i for i, feat in enumerate(feature_names) if feat in protected_attributes]
    non_protected_indices = [i for i, feat in enumerate(feature_names) if feat not in protected_attributes]
    
    for i, j in combinations(range(len(df)), 2):
        sample_i = df.iloc[i]
        sample_j = df.iloc[j]
        
        non_protected_match = True
        for idx in non_protected_indices:
            if sample_i.iloc[idx] != sample_j.iloc[idx]:
                non_protected_match = False
                break
        
        if not non_protected_match:
            continue
            
        protected_differ = False
        for idx in protected_indices:
            if sample_i.iloc[idx] != sample_j.iloc[idx]:
                protected_differ = True
                break
                
        if not protected_differ:
            continue
            
        if sample_i['prediction_class'] != sample_j['prediction_class']:
            x1 = lhs_scaled[i]
            x2 = lhs_scaled[j]
            
            distance = abs(sample_i['prediction'] - sample_j['prediction'])
            
            counterfactuals.append({
                'sample_1': x1,
                'sample_2': x2,
                'sample_1_pred': sample_i['prediction'],
                'sample_2_pred': sample_j['prediction'],
                'distance': distance
            })
            distances.append(distance)
    
    distance = np.mean(distances) if distances else 0.0
    return counterfactuals, distance
...
if __name__ == "__main__":
    ...
    counterfactuals, distance = generate_lhs_counterfactuals(
