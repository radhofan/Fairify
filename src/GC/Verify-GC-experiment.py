...
for model_file in tqdm(model_files, desc="Processing Models"):  # tqdm for model files loop
    ...
    if not (model_file.startswith("GC-5") or model_file.startswith("GC-10")):
        continue
    ...
    for p in tqdm(p_list, desc="Processing Partitions", total=len(p_list)):  # tqdm for partitions loop
        ...
        # Save metric to csv
        model_prefix = next((prefix for prefix in ["GC-5", "GC-10"] if model_file.startswith(prefix)), "unknown")
